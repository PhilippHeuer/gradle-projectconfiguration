...
interface PluginModule {
    companion object {
         ...
         */
        fun log(logLevel: LogLevel, project: Project, config: ProjectConfigurationExtension, message: String) {
            if (config.logLevel.isPresent) {
                if (config.logLevel.get() <= logLevel) {
                    println("$logLevel: [${project.name}] ${this::class.java.simpleName} -> $message")
                }
            } else {
                // delegate to slf4j
    ...
    fun log(logLevel: LogLevel, message: String) {
        log(logLevel, project, config, message)
    }
}